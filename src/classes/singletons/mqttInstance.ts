...
export class MqttInstance {
	...
	private static async init() {
		const applicationArr = await MongoClientInstance.getInstance()
		...
		applicationArr.forEach((value) => {
			...
			mqttClient.on('connect', () => {
				logger.info(
					`Connected to MQTT broker with address: ${application.mqttBrokerUrl}`,
					'mqtt',
					'connect',
					ForegroundColor.Green,
				);
				...
				logger.info(
					`Subscribed to all topics using: #`,
					'mqtt',
					'subscribe',
					ForegroundColor.Green,
				);
	...
	public publish(applicationId: string, topic: string, message: string) {
		if (!MqttInstance._mqttClientMap.has(applicationId)) {
			logger.error(
				`there is no mqtt client with applicationId ${applicationId}`,
				'mqtt',
				'publish',
			);
		}
		logger.info(
			`applicationId: ${applicationId}, topic: ${topic}, message: ${message}`,
			'mqtt',
			'publish',
		);
		MqttInstance._mqttClientMap
			?.get(applicationId)
